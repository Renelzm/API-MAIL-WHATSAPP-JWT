...
export const WhatsAppProvider = async ({ to, message, session }: whatsAppInterface) => {
  ...
  try {
    return await createSocket(to, message, messageSent, session)
  } catch (error) {
...
const createSocket = async (to?: any, message?: string, messageStatus?: boolean, session?: string): Promise<object> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      // >6 Proceso de generacion de QRs
      if (qr && !generetadQr) {
        ...
        reject("Tiempo de qr terminado");
      }
        // >6 Proceso de generacion de Conexion y envio de mensajes
      if (connection === 'close' && !messageSent && !terminarPoceso) {
        ...
        // >4 AQUI INICIA EL ENVIO DE MESNSAGES TIENE 3# OPCIONES CUADRAR A LAS NECESIDADES DE LA INSTITUCION
      ...
      // const testsocket = await socket.onWhatsApp(`5218713162338@s.whatsapp.net`)
      // // console.log(testsocket)
      //   
            // Validar con onWhatsApp invalidos y validos
            // for (const e of to) { 
            //   if (e.length === 10) {
            ...
            // solo enviar mensajes sin validaciones
